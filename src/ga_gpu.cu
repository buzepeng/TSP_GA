...
__global__ void fit_sum_kernel(World* pop, int pop_size, float* fit_sum)
{
	...
	*fit_sum = sum;
	...
	if (tid < pop_size)
	{
		...
		pop[tid].fit_prob = pop[tid].fitness;

		// if (tid == (pop_size - 1))	*fit_sum = sum;
	}
}

__device__ float max_fitsum_kernel(World* pop, int pop_size){
...
__device__ float min_fitsum_kernel(World* pop, int pop_size){
...
__global__ void fit_prob_kernel(World* pop, int pop_size, float* fit_sum)
{
	// Get the thread id
	int tid = getGlobalIdx_2D_1D();
	float fit_max = max_fitsum_kernel(pop, pop_size);
	float fit_min = min_fitsum_kernel(pop, pop_size);
	// printf("factor:%.10f\n", factor);
	// Evaluate if the thread is valid
	if (tid < pop_size)
		// pop[tid].fit_prob /= *fit_sum;
		pop[tid].fit_prob = (pop[tid].fit_prob - fit_min)/(fit_max - fit_min);
		// pop[tid].fit_prob /= fit_max;
	...
	int* sel_ix)
{
	...
	if (tid < (pop_size))
	{
		...
		for (int j=0; j<pop_size; j++)
		{
			...
			// if (0.6 <= pop[j].fit_prob)
			{
				sel_ix[tid] = j;
				break;
			}
		}
		// if(0.2<=pop[tid].fit_prob)	sel_ix[tid] = tid;
	}
}
